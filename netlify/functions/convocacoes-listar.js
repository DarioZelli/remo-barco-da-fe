  ...
  try {
    ...
    for (const blob of blobs) {
      try {
        ...
        convocacoes.push(item);
      } catch {
        // Ignora registro inválido
      }
    }

    convocacoes.sort((a, b) => new Date(b.criadoEm) - new Date(a.criadoEm));

    return {
      statusCode: 200,
      headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' },
      body: JSON.stringify({ total: convocacoes.length, convocacoes })
    };
  } catch (err) {
    console.error('Erro ao listar convocações:', err);
    return {
      statusCode: 500,
      headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' },
